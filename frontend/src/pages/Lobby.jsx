...
export default function Lobby({ user }) {
  ...
  function roomBetValue(room) {
    const v = room.bet_amount ?? room.bet ?? room.betValue ?? 0;
    const n = Number(v);
    return Number.isFinite(n) ? n : 0;
  }

  function balanceNumber() {
    const b = Number(me?.balance);
    return Number.isFinite(b) ? b : null;
  }

  function hasEnoughBalance(amount) {
    const b = balanceNumber();
    if (b == null) return true; // balance gelmiyorsa UI bloklamasın
    return b >= amount;
  }

  async function onCreateRoom() {
    const betAmount = normalizeBet(bet);

    if (!hasEnoughBalance(betAmount)) {
      setError("Yetersiz bakiye: Bu bet ile oda açamazsın.");
      return;
    }
    ...
    try {
      ...
      const newId = created?.id ?? created?.room?.id;

      await refreshAll();

      // oluşturunca direkt odaya gitmek mantıklı
  ...
  async function onJoinRoom(room) {
    const betAmount = roomBetValue(room);

    if (!hasEnoughBalance(betAmount)) {
      setError("Yetersiz bakiye: Bu odaya katılamazsın.");
      return;
    }
    ...
    try {
      ...
      // join sonrası oda full olmuşsa game ekranına git
      ...
      <div style={grid}>
        ...
        <section style={card}>
          ...
          <div style={row}>
            <input
              value={bet}
              onChange={(e) => setBet(normalizeBet(e.target.value))}
              type="number"
              min={1}
              style={input}
              placeholder="Bet"
            ...
            <button
              ...
              disabled={creating || !hasEnoughBalance(normalizeBet(bet))}
              type="button"
              title={!hasEnoughBalance(normalizeBet(bet)) ? "Yetersiz bakiye" : ""}
            >
              {creating ? "Oluşturuluyor..." : "Yeni Oda Aç"}
            </button>
          </div>
            ...
            <table style={table}>
              ...
              <tbody>
                {filteredRooms.map((r) => {
                  ...
                  const betValue = roomBetValue(r);
                  const canAfford = hasEnoughBalance(betValue);
                      ...
                      <td style={td}>
                        {st === "finished" ? (
                          ...
                          <button
                            ...
                            disabled={!canAfford || joiningId === r.id}
                            type="button"
                            title={!canAfford ? "Yetersiz bakiye" : ""}
                          >
                            {joiningId === r.id ? "Katılınıyor..." : "Katıl"}
                          </button>
                        ) : (
                          // FULL: direkt oyun ekranına git (Link yerine buton: daha deterministik)
                          <button style={linkBtn} onClick={() => onPlay(r)} type="button">
                            Play (WS)
                          </button>
                        )}
                      </td>
        ...
        <section style={card}>
          ...
          <div style={{ marginTop: 10, opacity: 0.8 }}>
            Test: <Link to="/room/12">/room/12</Link>
          </div>
        </section>
      </div>
    </div>
